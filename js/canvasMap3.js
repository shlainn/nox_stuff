$(document).ready(function()
{
    ...
    $("#canvasmap").click(function(event) { canvasmap_click(event); });
    $("#canvasmap").mousemove(function(event) { canvasmap_tooltip(event); });
    $("#canvasmap").mouseover(function() { canvasmap_tooltip_start(); });
    $("#canvasmap").mouseout(function() { canvasmap_tooltip_stop(); });
    ...
    var scan_animation_start, scan_animation_duration, scan_animation_step, scan_animation_stop = false;
    ...
    function getVisibleFleets(fleetId, x, y, z, rangeOfSight)
    {
        $.ajax(
        {
            ...
            success: function(dataArray)
            {
                if (dataArray.length > 0)
                {
                    var fleetData =  jQuery.parseJSON(dataArray);
                    
                    // "myFleets", "unionFleets", "alienFleets", "foreignFleets"
                    // we only get number of foreign fleets if we are too far away
                    var foreignFleets = fleetData.foreignFleets;
                    if ($.isArray(foreignFleets)) {
                        fleets = foreignFleets.concat(fleetData.unionFleets, fleetData.alienFleets);
                    } else {
                        fleets = [];
                    }
                    myfleets = fleetData.myFleets;
                }
                scan_animation_stop = 1;
                draw();
            },
    ...
    var cc=document.getElementById("canvasmap_click");
    ...
    function canvasmap_tooltip(e)
    {
      ...
      var parentOffset = $("#canvasmap").offset(); 
      ...
      if(fleet_id != -1) 
      {
        ...
        for(var i = 0; i < index.length; i += 1) {
          if(index[i] > 0) {
            this_fleet = fleets[index[i]-1];
          } else {
            this_fleet = myfleets[-1*index[i] -1];
          }
    ...
    // ctx.globalAlpha = 0.6;
      ...
      grd.addColorStop(0.95,"rgba(0,255,0,1)");
      grd.addColorStop(1,"rgba(0,0,0,0)");
      ctx.fillStyle=grd;//"rgba(255,0,0,0.6)";
      ctx.beginPath();
      ctx.scale(1,500/630);
    ...
    function scan_grid(time){
      ...
      if(scan_animation_step === 0) {
        animation_progress = ((time-scan_animation_start) / 1000)%1;
    ...
    function draw_fleet_numbers_color2() 
    {
      for(var type in fleet_sectors) {
        ...
        for(var sector in fst) 
        {
            ...
            var grd=ctx.createRadialGradient(0,0,0,0,0,75);
            if(type == "alienFleets") {
              grd.addColorStop(0,"rgb(255,"+Math.floor((fst[sector] / max_in_sector)*150)+",0)");
              grd.addColorStop(1,"rgba(100,0,0,0)");
            } else {
              grd.addColorStop(0,"rgb(0,"+Math.floor((fst[sector] / max_in_sector)*255)+",0)");
              grd.addColorStop(1,"rgba(0,10,0,0)");
            }
            ctx.fillStyle=grd;//"rgba(255,0,0,0.6)";
            ctx.beginPath();
            ctx.scale(1,0.397);
    ...
    function draw_fleet_positions() 
    {
      ...
      {
        fleetpos = rotate_around_current(fleets[i]);
        ...
        if(screen_x < 10 || screen_y-y_offset < 10)
          continue;
        ctx.fillRect(Math.floor(screen_x),Math.floor(screen_y-y_offset),5,5);
        ...
        if(fleet_id == -1) { //no other fleet here
          ...
          index_number += 1;
          click_ctx.fillRect(Math.floor(screen_x),Math.floor(screen_y-y_offset),5,5);
        } else { //Other fleet here
          fleet_index[fleet_id].push(i+1);
        }
      }
      ...
      {
        fleetpos = rotate_around_current(myfleets[i]);

        screen_x = 10+(fleetpos.rot_x-min.x)*(610/scale);
        screen_y = 250+(fleetpos.rot_z-min.z)*(240/scale);
        y_offset = (fleetpos.rot_y-min.y)*(250/scale);

        if(screen_x > 620 || screen_y > 490 || screen_y-y_offset > 490)
          continue;
        if(screen_x < 10 || screen_y < 250 || screen_y-y_offset < 10)
          continue;
        ctx.fillRect(Math.floor(screen_x),Math.floor(screen_y-y_offset),5,5);
        ...
        if(fleet_id == -1) { //no other fleet here
          ...
          fleet_index[fleet_id].push(-1*(i+1));
        }
      }
    }
